...
from phone_agent.amap import get_amap_client, NavigationRoute
...
class ActionResult:
    ...
    navigation_data: Optional[dict] = None  # For navigation results
...
class ActionHandler:
    ...
    def _get_handler(self, action_name: str) -> Callable | None:
        ...
        handlers = {
            ...
            "Call_Navigation": self._handle_call_navigation,
    ...
    def _handle_call_navigation(self, action: dict, width: int, height: int) -> ActionResult:
        """
        Handle navigation planning action.

        Expected action format:
        {
            "action": "Call_Navigation",
            "destination": "目的地名称或地址",
            "origin": {"lon": xxx, "lat": yyy}  # optional
        }
        """
        destination = action.get("destination")
        if not destination:
            return ActionResult(False, False, "Missing destination for navigation")

        # Get origin from action or use default
        origin = action.get("origin")

        try:
            amap_client = get_amap_client()

            # If origin not provided, try to geocode destination first
            if not origin:
                # Geocode destination to get coordinates
                dest_coords = amap_client.geocode(destination)
                if not dest_coords:
                    return ActionResult(False, False, f"Cannot find location: {destination}")

                # For now, return a response indicating route planning is needed
                # In a real scenario, origin would come from user's current location
                return ActionResult(
                    True,
                    False,
                    f"Navigation planned to {destination}",
                    navigation_data={
            ...
            # Plan route with origin and destination
            route = amap_client.plan_walking_route(origin, destination)

            if not route:
                return ActionResult(False, False, f"Failed to plan route to {destination}")

            # Convert route to navigation-friendly format
            steps_data = []
            ...
            return ActionResult(
                True,
                False,
                f"Route planned: {len(route.steps)} steps, {route.total_distance}m total",
                navigation_data={
                    "destination": destination,
            ...
            return ActionResult(False, False, f"Navigation failed: {e}")
